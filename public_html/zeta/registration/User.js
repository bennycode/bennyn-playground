// Generated by CoffeeScript 1.6.3
/*
  Docs:
  https://docs.z-infra.com/dev-device-api/latest/reference/users.html
*/


(function() {
  var User,
    __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; };

  namespace({
    Zeta: {
      Registration: User = (function() {
        function User(name, email, password) {
          this.get_registration_payload = __bind(this.get_registration_payload, this);
          this.name = new Zeta.Registration.UserProperty(name);
          this.email = new Zeta.Registration.UserEmailAddress(email);
          this.password = new Zeta.Registration.UserPassword(password);
        }

        User.prototype.optional = {
          accent: [],
          picture: [],
          phone: "",
          phone_code: ""
        };

        User.prototype.get_registration_payload = function() {
          var key, payload;
          payload = {};
          for (key in this) {
            if (this[key] instanceof Zeta.Registration.UserProperty && this[key].is_valid()) {
              payload[key] = this[key].value;
            }
          }
          return JSON.stringify(payload);
        };

        User.prototype.has_valid_registration_data = function() {
          if (this.name.is_valid() && this.email.is_valid() && this.password.is_valid()) {
            return true;
          } else {
            return false;
          }
        };

        return User;

      })()
    }
  });

}).call(this);

/*
//@ sourceMappingURL=User.map
*/
